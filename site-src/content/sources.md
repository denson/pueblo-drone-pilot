---
slug: sources
title: Sources and downloads
description: Every claim on this site traces to the research report of August 23, 2026, and through it to official grant, FAA, and manufacturer pages, each checked on a stated date. The full report is downloadable here. Time-sensitive statuses must be rechecked on the date they matter.
eyebrow: Every claim, linked and dated
---
# Sources and downloads

## The research reports

Two reports, both dated August 23, 2026, both published with the host business unnamed by design:

- **[report-vlos.md](../report-vlos.md)**, the current plan: a low-altitude, geofenced, Part 107 visual-line-of-sight pilot that can start before any waiver, with the three-stage operating model and the six phases. [[verified]]
- **[report.md](../report.md)**, the original concept study: the long-endurance county aircraft, the full grant analysis, and the aircraft appendix; now the reference for the later remote-launch phase. [[verified]]

Where they differ, the low-altitude report governs the current plan.

## How to read the claim states

Everything on this site carries one of four states, shown as a labeled chip: [[verified]] a fact with a source, checked on the stated date; [[estimate]] a planning figure, labeled with where it came from; [[proposal]] something suggested and not yet decided by anyone; [[open]] a question nobody has answered yet. A time-sensitive status (a Blue-list entry, a grant deadline, a price) is only as good as its check date and must be reverified when it matters.

## Official sources

- Grants: <a href="https://cops.usdoj.gov/cpdmicrogrants">COPS CPD Microgrants</a> · <a href="https://bja.ojp.gov/funding/uas">BJA UAS requirements</a> · <a href="https://bja.ojp.gov/program/jag/archives">Byrne JAG archives</a> · <a href="https://dhsem.colorado.gov/grants/grant-programs/homeland-security-grant-program">Colorado HSGP</a> · <a href="https://www.fema.gov/grants/preparedness/emergency-management-performance">FEMA EMPG</a>
- Procurement: <a href="https://bluelist.dcma.mil/">DCMA Blue List portal</a> · <a href="https://www.diu.mil/blue-uas-portal">DIU Blue UAS portal</a>
- FAA: <a href="https://www.faa.gov/uas/public_safety_gov/drone_program">public-safety drone programs</a> · <a href="https://www.faa.gov/uas/public_safety_gov/public_safety_toolkit">Public Safety Toolkit</a> · <a href="https://www.faa.gov/uas/commercial_operators/part_107_waivers">Part 107 waivers</a> · <a href="https://www.faa.gov/uas/advanced_operations/part_91_waivers">Part 91 waivers</a>
- Part 107 rules: <a href="https://www.ecfr.gov/current/title-14/chapter-I/subchapter-F/part-107/subpart-B/section-107.31">107.31 VLOS</a> · <a href="https://www.ecfr.gov/current/title-14/chapter-I/subchapter-F/part-107/subpart-B/section-107.51">107.51 limits</a> · <a href="https://www.ecfr.gov/current/title-14/chapter-I/subchapter-F/part-107/subpart-B/section-107.29">107.29 night</a> · <a href="https://www.faa.gov/uas/getting_started/remote_id">Remote ID</a> · <a href="https://www.faa.gov/uas/commercial_operators/part_107_waivers/waivers_issued">issued waivers</a>
- Local capability: <a href="https://www.pueblosheriff.com/245/Patrol-Services-Division">Sheriff's S.O.A.R.R.</a> · <a href="https://www.pueblosheriff.com/Archive.aspx?ADID=5493">2024 accountability report</a> · <a href="https://www.pueblo.us/2435/UAV-Unit">City UAV unit</a> · <a href="https://www.koaa.com/news/covering-colorado/pueblo-sheriffs-office-introduces-aerial-response-and-rescue-team">KOAA on S.O.A.R.R.</a>
- Candidate aircraft: <a href="https://skyfront.com/perimeter-8">Skyfront Perimeter 8</a> · <a href="https://skyfront.com/solutions/surveillance-and-security">Skyfront surveillance and security</a> · <a href="https://harrisaerial.com/carrier-drones/carrier-h6-hybrid/">Harris Carrier H6 Hybrid</a> · <a href="https://support.harrisaerial.com/maintenance-repairs/">Harris maintenance and support</a> · <a href="https://www.pdw.ai/products/c100-public-safety">PDW C100 Public Safety</a> · <a href="https://www.inspiredflight.com/if800">Inspired Flight IF800 Tomcat</a> · <a href="https://shop.inspiredflight.com/products/if800-tomcat">IF800 loaded endurance</a> · <a href="https://docs.inspiredflight.com/inspired-documentation/products/aircraft/if800-tomcat/if800-tomcat-overview/if800-specifications">IF800 specifications</a> · <a href="https://www.inspiredflight.com/if1200">Inspired Flight IF1200</a>
- Payloads: <a href="https://docs.gremsy.com/payloads/vio/general">Gremsy VIO F1</a> · <a href="https://www.trilliumeng.com/news/trillium-engineerings-hd45-gimbal-achieves-auvsi-green-uas-certification">Trillium HD45 Green UAS certification</a>
- Future benchmark systems: <a href="https://www.khaero.com/k1000ule">K1000ULE</a> · <a href="https://shield.ai/v-bat/">Shield AI V-BAT</a> · VXE30 Stalker (manufacturer page cited in the report no longer resolves; unlinked as of 2026-08-24)

All checked 2026-08-23, and every link above rechecked 2026-08-24; the full register is in the reports. Two vendor pages (Shield AI, Harris support) refuse automated requests but open normally in a browser, and one (the VXE30 manufacturer page) no longer resolves at the address the report recorded. [[verified]]

## Change log

- **2026-08-24**: revised around the scaled-down low-altitude plan (report-vlos.md): VLOS Part 107 first, the long-endurance county aircraft and BVLOS moved to the later phase.
- **2026-08-24**: site published from the research report of 2026-08-23.

<!-- agent-only -->

## Notes for agents on this page

- The report is the canonical source; the site is its readable form. Where the site compresses, the report governs.
- "Checked 2026-08-23" is the research date. Grant deadlines, Blue-list status, and prices change; a restatement on a later date should say when the claim was last checked.
- The change log on this page is the site's own history; page-level dates come from each page's revision history, machine-checkable in git.
- Author: Denson Smith, volunteer researcher. The site is not published by, reviewed by, or endorsed by Pueblo County, the Sheriff's Office, the City of Pueblo, the FAA, or any grant agency.
